...
from src.checkpoint import load_trained_state_with_offbook_migration
...
class ModelTests(unittest.TestCase):
  ...
  def test_offbook_migration_is_exact_zero_initialized_profile_identity(self):
    torch.manual_seed(23)
    legacy = ProfileConditionedLossModel(ModelConfig(), "full-31").eval()
    torch.manual_seed(24)
    migrated = OffbookProfileConditionedLossModel(ModelConfig(), "full-31").eval()
    migration = load_trained_state_with_offbook_migration(migrated, legacy.state_dict())
    actual_time = torch.full((2, 3), 1500.0)
    profile_values = torch.randn(2, 3, 31)
    profile_missing = torch.rand(2, 3, 31) > 0.7
    offbook = torch.zeros(2, 3)
    with torch.no_grad():
      old = legacy(*inputs(), actual_time, profile_values, profile_missing)
      new = migrated(*inputs(), actual_time, offbook, profile_values, profile_missing)
    self.assertTrue(migration["migratedOffbookCheckpoint"])
    self.assertTrue(torch.equal(old.pred_time_log_seconds, new.pred_time_log_seconds))
    self.assertTrue(torch.equal(old.severity_logits, new.severity_logits))
    self.assertTrue(torch.equal(old.wld_logits, new.wld_logits))

  def test_offbook_changes_only_severity_and_wld_after_training_branch_is_nonzero(self):
    torch.manual_seed(29)
    model = OffbookProfileConditionedLossModel(ModelConfig(), "full-31").eval()
    with torch.no_grad():
      model.offbook_severity_film.weight.fill_(0.02)
      first = model(*inputs(), torch.ones(2, 3), torch.zeros(2, 3), torch.zeros(2, 3, 31), torch.zeros(2, 3, 31, dtype=torch.bool))
      second = model(*inputs(), torch.ones(2, 3), torch.ones(2, 3), torch.zeros(2, 3, 31), torch.zeros(2, 3, 31, dtype=torch.bool))
    self.assertTrue(torch.equal(first.pred_time_log_seconds, second.pred_time_log_seconds))
    self.assertFalse(torch.equal(first.severity_logits, second.severity_logits))
    self.assertFalse(torch.equal(first.wld_logits, second.wld_logits))

  def test_offbook_input_is_required_and_shape_checked(self):
    model = OffbookProfileConditionedLossModel(ModelConfig()).eval()
    with self.assertRaises(TypeError):
      model(*inputs(), torch.ones(2, 3), torch.zeros(2, 3, 31), torch.zeros(2, 3, 31, dtype=torch.bool))
    with self.assertRaisesRegex(ValueError, "offbook feature shape"):
      model(*inputs(), torch.ones(2, 3), torch.ones(2, 2), torch.zeros(2, 3, 31), torch.zeros(2, 3, 31, dtype=torch.bool))
