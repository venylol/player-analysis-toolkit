...
Every newly materialized board-CNN bundle carries the scalar contract arrays
`board_perspective=snapshot_side_to_move_v1` and
`board_feature_revision=board_context_snapshot_side_to_move_v1__23_planes_v1`.
For each of `current`, `prev_opponent`, and `prev_own`, token `2` means the
selected snapshot row's own mover and token `3` means that row's opponent.
The selected row's recorded `side_to_move` is mandatory; pass-safe behavior must
never be inferred from ply parity. Token `0` remains missing-context padding and
token `1` remains an empty square. Legacy model-ready files without this metadata
retain fixed black/white X/O semantics and are not valid inputs to new training.
