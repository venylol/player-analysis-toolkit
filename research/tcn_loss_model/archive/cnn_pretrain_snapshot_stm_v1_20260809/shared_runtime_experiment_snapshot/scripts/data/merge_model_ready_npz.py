...
CONSTANT_ARRAYS = {
    "input_features", "board_cnn_channels", "preprocessing_sha256", "input_policy",
    "board_perspective", "board_feature_revision",
}
...
def main() -> int:
    ...
    with np.load(args.base_data.resolve(), allow_pickle=False) as base, np.load(
        ...
        required_board_contract = {"board_perspective", "board_feature_revision"}
        for label, source in (("base", base), ("extension", extension)):
            missing_contract = sorted(required_board_contract - set(source.files))
            if missing_contract:
                raise ValueError(
                    f"{label} data lacks explicit board contract arrays: {missing_contract}"
                )
    ...
    manifest = {
        ...
        "boardPerspective": str(arrays["board_perspective"].item()),
        "boardFeatureRevision": str(arrays["board_feature_revision"].item()),
