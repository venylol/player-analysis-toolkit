...
ROOT = next(
    (candidate for candidate in SCRIPT_PATH.parents if (candidate / "src").is_dir()),
...
def make_model_ready_arrays(
    ...
    # Do not call the dynamically imported historical builder here. It encodes
    # fixed black/white X/O tokens. The maintained builder normalizes every
    # selected snapshot with that source row's own side_to_move.
    board_tokens, board_move_tokens = make_board_context_sequences(frame, games, workers)
    ...
    arrays.update({
        ...
        "board_feature_revision": np.asarray(
            "board_context_snapshot_side_to_move_v1__23_planes_v1"
        ),
    })
    return arrays
...
def main() -> int:
    ...
    manifest = {
        ...
        "boardFeatureRevision": "board_context_snapshot_side_to_move_v1__23_planes_v1",
        "inputs": {
