...
class ModelConfig:
    ...
    board_cnn_architecture: str = "residual-v2"
    ...
    def from_checkpoint(cls, checkpoint: dict) -> "ModelConfig":
        ...
        return cls(
            ...
            residual_blocks=int(cfg.get("residual_blocks", DEFAULT_RESIDUAL_BLOCKS)),
            embedding_projection_kernel=int(cfg.get("embedding_projection_kernel", DEFAULT_PROJECTION_KERNEL)),
            board_cnn_architecture=str(cfg.get("board_cnn_architecture", "legacy-three-conv-v1")),
        )
...
class LegacyBoardCNNEncoder(nn.Module):
    """Original three-convolution encoder retained only for unmodified old checkpoints."""
...
class BoardCNNEncoder(nn.Module):
    """One shared 23-channel residual CNN for all per-timestep board context."""

    input_channels = 23

    def __init__(self, cfg: ModelConfig) -> None:
        super().__init__()
        if cfg.board_embedding_dim != BOARD_EMBEDDING_DIM:
            raise ValueError(f"board_embedding_dim must be {BOARD_EMBEDDING_DIM}")
        self.current_hint_planes = cfg.current_hint_planes
        self.current_hint_value_planes = current_hint_value_plane_count(cfg.current_hint_planes)
        self.prev_own_hint_value_planes = PREV_OWN_HINT_VALUE_PLANES
        calculated_channels = (
            len(BOARD_BASE_CNN_CHANNELS) + self.current_hint_planes
            + self.current_hint_value_planes + self.prev_own_hint_value_planes
        )
        if calculated_channels != self.input_channels:
            raise ValueError(f"formal board channel contract must contain 23 planes, got {calculated_channels}")
        self.shared = SharedBoardCNN(
            input_channels=self.input_channels,
            board_channels=cfg.board_channels,
            residual_blocks=cfg.residual_blocks,
            board_embedding_dim=cfg.board_embedding_dim,
            embedding_projection_kernel=cfg.embedding_projection_kernel,
        )
    ...
    def build_input_planes(
        ...
        for value_idx in range(self.current_hint_value_planes):
            ...
            planes.append(plane)
        planes.extend(prev_values[:, idx].unsqueeze(1).expand(-1, 64) for idx in range(2))
        board_planes = torch.stack(planes, dim=1).reshape(batch * seq_len, self.input_channels, 8, 8)
        return board_planes, batch, seq_len

    def state_only_features(self, board_planes: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Second-pass auxiliary representation with channels 3..22 strictly zero."""
        if board_planes.ndim != 4 or tuple(board_planes.shape[1:]) != (self.input_channels, 8, 8):
            raise ValueError(f"expected Bx23x8x8 input, got {tuple(board_planes.shape)}")
        state_only = torch.zeros_like(board_planes)
        state_only[:, :3] = board_planes[:, :3]
        spatial = self.shared.spatial_features(state_only)
        return spatial, self.shared.embedding_from_spatial(spatial)

    def state_only_auxiliary_forward(
        self, board_planes: torch.Tensor, auxiliary_heads: nn.Module,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Training-only auxiliary forward that cannot observe history or hint planes."""
        spatial, embedding = self.state_only_features(board_planes)
        return auxiliary_heads(spatial, embedding)

    def forward(self, board_tokens: torch.Tensor, board_move_tokens: torch.Tensor,
                current_hint_tokens: torch.Tensor, current_hint_values: torch.Tensor,
                prev_own_hint_values: torch.Tensor) -> torch.Tensor:
        board_planes, batch, seq_len = self.build_input_planes(
            board_tokens, board_move_tokens, current_hint_tokens,
            current_hint_values, prev_own_hint_values,
        )
        return self.shared(board_planes).reshape(batch, seq_len, -1)
...
class BoardConditionedBackbone(nn.Module):
    ...
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        self.board_encoder = (
            LegacyBoardCNNEncoder(cfg)
            if cfg.board_cnn_architecture == "legacy-three-conv-v1"
            else BoardCNNEncoder(cfg)
        )
