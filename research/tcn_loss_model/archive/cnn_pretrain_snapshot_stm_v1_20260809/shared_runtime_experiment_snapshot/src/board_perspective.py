"""Canonical board-token construction for formal snapshot-side-to-move models."""
...
LEGACY_BOARD_PERSPECTIVE = "legacy_fixed_color"
SUPPORTED_BOARD_PERSPECTIVES = frozenset({BOARD_PERSPECTIVE, LEGACY_BOARD_PERSPECTIVE})
BOARD_CONTEXT_NAMES = ("current", "prev_opponent", "prev_own")
BOARD_TOKEN_IDS = {"padding": 0, "empty": 1, "local_mover_X": 2, "local_opponent_O": 3}
...
def make_board_context_token_sequences(
    ...
    *,
    ...
    """Build board tokens when move-coordinate context is not needed."""
    ...
    for game_index, game_id in enumerate(requested_games):
        ...
        local_boards = [
            encode_snapshot_board(board, side)
            for board, side in zip(view["board"], sides, strict=True)
        ]
        for position, side in enumerate(sides):
            boards[game_index, position, 0] = local_boards[position]
            ...
            for context, source_position in ((1, previous_opponent), (2, previous_own)):
                if source_position is None:
                    continue
                boards[game_index, position, context] = local_boards[source_position]
...
def make_board_context_sequences(
    ...
    *,
    ...
    """Build all three contexts using each selected source row's own side_to_move.

    ``_workers`` is accepted for compatibility with the historical builder. The
    direct implementation is intentionally deterministic and bounded by 60 rows/game.
    """
