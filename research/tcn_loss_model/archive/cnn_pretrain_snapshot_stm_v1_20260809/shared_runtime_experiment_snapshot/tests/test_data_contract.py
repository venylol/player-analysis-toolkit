...
class DataContractTests(unittest.TestCase):
  ...
  def test_model_ready_bundle_carries_order_and_preprocessing_identity(self):
    ...
    np.savez(
        ...
        board_perspective=np.array("snapshot_side_to_move_v1"),
        board_feature_revision=np.array("board_context_snapshot_side_to_move_v1__23_planes_v1"),
    )
    report = validate_model_ready_npz(
        ...
        expected_board_perspective="snapshot_side_to_move_v1",
    )
    ...
    self.assertEqual(report["boardPerspective"], "snapshot_side_to_move_v1")
    with self.assertRaisesRegex(ValueError, "board perspective mismatch"):
      validate_model_ready_npz(path, expected_board_perspective="legacy_fixed_color")
