#!/usr/bin/env python3
"""Resume-safe 450-game OQ Reference expansion using the audited batch pipeline."""
...
CURRENT_REFERENCE = DATA / "oq_elo_matchup400_reference_level22_1600plus_20260815"
CURRENT_SNAPSHOT_DETAILS = (
    CURRENT_REFERENCE
    / "provenance"
    / "oq_snapshot_reference400_20260815"
    / "acquisition"
    / "game_details.jsonl"
)
SNAPSHOT_NAME = "oq_snapshot_reference450_20260820"
BATCH_ID = "oq-reference450-20260820-once"
...
def load_current_reference() -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    if (
        ...
        raise ValueError("current matchup400 bundle is not a unique, aligned source")
...
def load_batch(output: Path) -> dict[str, Any]:
    ...
    if path.is_file():
        ...
        if value.get("batchId") != BATCH_ID:
            raise ValueError("conflicting OQ 450 snapshot batch already exists")
        return value
    return {
        "schema": "oq-reference450-one-shot-batch-v1",
        "batchId": BATCH_ID,
...
def configure() -> None:
    ...
    pipeline.DEFAULT_OUTPUT = DATA / "oq_elo_matchup450_reference_level22_1600plus_20260820"
    pipeline.PRIOR_FORMAL_MAXIMUM_ELO = 2495
    pipeline.TOP_BIN_LOWER = 2400
    pipeline.TARGET_PER_PAIR = 450
    pipeline.SEED = 20260820450
...
def finalize_leaderboard(args: Any) -> int:
    """Freeze the one batch after its finite missing-page retries are terminal."""
    ...
    for page in missing:
        pipeline.append_jsonl(terminal_failure_path, {
            ...
            "retryPolicy": "five_resume-retry rounds within the same full leaderboard batch",
    ...
    pipeline.atomic_json(leaderboard_json, {
        "schema": "oq-reference450-leaderboard-terminal-status-snapshot-v1",
